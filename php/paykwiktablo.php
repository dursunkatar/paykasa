<div class="table-responsive">
    <table class="table table-bordered table-striped table-sm" width="100%" cellspacing="0">
        ...
        $stmt = $db->query("SELECT
                                    ...
                                    pay.OlmasiGereken,
                                    pay.Kar,
                                    (SELECT COUNT(0) FROM urunler WHERE  urunler.KasaID=4 and urunler.Miktar=pay.Cashixir1 AND urunler.Satildimi=1 ) AS Satis,
                                    (SELECT COUNT(0) FROM urunler WHERE  urunler.KasaID=4 and urunler.Miktar=pay.Cashixir1 AND urunler.Satildimi=0  ) AS Basilmis
                                    FROM paykwik AS pay order by pay.Cashixir1");

        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)):?>
            <tr>
                <td><input name="idler" value="<?php echo $row['ID'] ?>" type="checkbox"/></td>
                <td><?php echo $row['Cashixir1'] ?> €</td>
